...
pub trait DataChannel: crate::sealed::Sealed + Send + Sync + 'static {
    ...
    fn id(&self) -> Option<RTCDataChannelId>;
...
pub(crate) struct DataChannelImpl {
    ...
    /// Cached stream identifier; `None` until the core assigns one on the connected procedure.
    /// Cached so the synchronous `id()` can serve it without acquiring the async core lock.
    id: std::sync::Mutex<Option<RTCDataChannelId>>,
...
impl DataChannelImpl {
    ...
    /// Records the stream identifier once the core has assigned it (on the connected procedure
    /// / SCTP connection), so `id()` can serve it synchronously.
    pub(crate) fn set_id(&self, id: RTCDataChannelId) {
