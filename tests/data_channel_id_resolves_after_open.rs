...
//! and stay stable  -  served synchronously from the impl's cached value without the async core
//! lock.
//!
...
use webrtc::data_channel::{DataChannel, DataChannelEvent, RTCDataChannelId, RTCDataChannelInit};
...
async fn run() -> Result<()> {
    ...
    // stale `None` cache.
    assert_eq!(dc.id(), Some(resolved), "id() must be stable once assigned");
